...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    beta.append(learning_rate * (y_i - sign(state(x_i, kernel))))
    X.append(x_i)
    return lambda x, y : sum([beta[i] * y(x, X[i]) for i in range(len(X))])
    # *** END CODE HERE ***
...
def rbf_kernel(a, b, sigma=1):
    ...
    distance = np.subtract(a,b).dot(np.subtract(a,b))
