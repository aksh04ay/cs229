...
def backward_cross_entropy_loss(probabilities, labels):
    ...
    for i in range(n):
        d_probs[i] += (-1 * labels[i] / probabilities[i])
...
def backward_prop(data, labels, params):
    ...
    output_loss = backward_cross_entropy_loss(y, labels)
    output_softmax = backward_softmax(logits, output_loss)
    w2_backward, b2_backward, data_backward = backward_linear(W2, b2, flattened, output_softmax)
    flattened_max_pool_forward = np.reshape(first_max_pool, (-1))
    output_relu = backward_relu(flattened_max_pool_forward, data_backward)
    output_maxpool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, output_relu.reshape(CONVOLUTION_FILTERS, MAX_POOL_SIZE, MAX_POOL_SIZE))
    w1_backward, b1_backward, data1_backward = backward_convolution(W1, b1, data, output_maxpool)
